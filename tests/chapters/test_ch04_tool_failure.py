...
from chapters import ch04_tool_failure
...
def test_the_tool_really_raises() -> None:
    ...
    with pytest.raises(ch04_tool_failure.PriceServiceError):
        ch04_tool_failure.price_of.invoke({"item": "milk"})


def test_the_failure_is_recorded_as_a_decision_we_took() -> None:
    trace = ch04_tool_failure.run()
