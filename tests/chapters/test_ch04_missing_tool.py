...
from chapters import ch04_missing_tool
...
def test_no_tool_covers_the_question_that_was_asked() -> None:
    ...
    assert "delivery" in ch04_missing_tool.USER_PROMPT
    assert [tool.name for tool in ch04_missing_tool.DECLARED_TOOLS] == ["stock_on_hand"]
...
def test_the_tool_call_succeeded_and_answered_a_different_question() -> None:
    (tool,) = ch04_missing_tool.run().find_spans("tool")
...
def test_the_schema_and_the_lookup_allow_exactly_the_same_items() -> None:
    permitted = set(get_args(ch04_missing_tool.GroceryItem.__value__))
    assert permitted == set(ch04_missing_tool.STOCK_ON_HAND)
